...
import { getRandomEnumEntry, Vector3, VehiclesHash, WeatherEnum, WeatherTypeHash } from '../shared';
...
export namespace World {
    let currentCloudHat: CloudHat;

    /**
     * Get the current camera that's rendering.
     *
     * @return The camera that's currently used.
     */
     ...
     *
     * @return {CloudHat} The current cloud hat type.
     */
     ...
     *
     * @return The current cloud opacity between 0.0 and 1.0
     */
     ...
     *
     * @return {WeatherTypeHash} The current type of weather.
     */
     ...
     *
     * @return {WeatherTypeHash} Returns the hash name of the next weather type.
     */
     ...
     *
     * @return {Blip|null} The created blip if the entity exists, otherwise null.
     */
    export function createEntityBlip(entity: Entity): Blip {
     ...
     *
     * @return {Promise<Ped>} A promise that resolves to the created Ped instance, or null if the model is invalid or unavailable.
     */
    export async function createPed(model: Model, position: Vector3, heading = 0): Promise<Ped> {
     ...
     * @return {Vehicle} Vehicle object.
     */
    export async function createRandomVehicle(position: Vector3, heading = 0): Promise<Vehicle> {
     ...
     *
     * @return {Prop[]} An array of props that exist.
     */
    export function getAllProps(): Prop[] {
        const props: Prop[] = [];
        const [handle, entityHandle] = FindFirstObject(null) as [number, number];
        ...
        do {
            findResult = FindNextObject(entityHandle, null);
            if (findResult[0]) {
     ...
     *
     * @return {Ped[]} An array of `Ped` objects representing all pedestrians found.
     */
    export function getAllPeds(): Ped[] {
        const peds: Ped[] = [];
        const [handle, entityHandle] = FindFirstPed(null) as [number, number];
     ...
     *
     * @return {Vehicle[]} An array of all vehicles found.
     */
    export function getAllVehicles(): Vehicle[] {
        const vehicles: Vehicle[] = [];
        const [handle, entityHandle] = FindFirstVehicle(null) as [number, number];
