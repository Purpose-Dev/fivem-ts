import { Vehicle, VehicleMod, VehicleToggleMod } from '.';
import {
    LicensePlateStyle,
    LicensePlateType,
    VehicleColor,
...
import { VehicleWheelType, VehicleWindowTint } from '../../shared';
...
export class VehicleModCollection {
     ...
     * @return {boolean} - True if the vehicle has the specified modification, false otherwise.
     */
     ...
     * @return {VehicleMod} The instance of VehicleMod corresponding to the provided modType.
     */
    public getMod(modType: VehicleMods): VehicleMod {
     ...
     * @return {VehicleToggleMod} The toggle modification for the specified type.
     */
     ...
     *
     * @return {VehicleMod[]} An array of VehicleMod objects representing all the vehicle modifications.
     */
     ...
     *
     * @return {boolean} True if all neon lights are on, false otherwise.
     */
     ...
     * @return {boolean} True if the vehicle has the specified neon light, false otherwise.
     */
     ...
     * @return {boolean} - Returns true if the neon light is on, false otherwise.
     */
    ...
    public setAllNeonLightsOn(): void {
        Object.values(VehicleNeonLight).forEach((light: VehicleNeonLight) => {
            this.setNeonLightOn(light, true);
        });
    }
    ...
    public setAllNeonLightsOff(): void {
        Object.values(VehicleNeonLight).forEach((light: VehicleNeonLight) => {
            this.setNeonLightOn(light, false);
        });
    }
    ...
    public get Livery(): number {
        if (this.LiveryCount > 0) {
            return this.getMod(VehicleMods.LiveryMod).Index;
        }
