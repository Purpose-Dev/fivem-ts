...
export class VehicleDoorCollection {
     ...
     *
     * @return {VehicleDoor} The door at the specified index.
     */
    public getDoor(index: VehicleDoorIndex): VehicleDoor {
        if (!this.vehicleDoors.has(index)) {
            this.vehicleDoors.set(index, new VehicleDoor(this.owner, index));
        }
        return this.vehicleDoors.get(index);
    }

    /**
     * Retrieves all doors associated with the vehicle.
     *
     * @return {VehicleDoor[]} An array containing all valid vehicle doors.
     */
     ...
     *
     * @return {boolean} True if the vehicle has the specified door, false otherwise.
     */
