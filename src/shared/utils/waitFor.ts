 ...
 *
 * @param cb - A callback function that returns a value of type `T` or a `Promise` of type `T`.
 *              The function will be called repeatedly until it returns a non-`undefined` value.
 * @param errorMessage - An optional custom error message to use if the timeout is reached before
 *                     the callback returns a value. Defaults to `'Callback failed to resolve'`.
 * @param timeout - An optional timeout duration in milliseconds. If `false`, it disables the
 ...
 *
 * @returns A promise that resolves with the non-`undefined` value returned by the callback,
 *          or rejects with an error if the timeout is reached before a value is returned.
 *
 * @throws Error if the timeout is reached before the callback returns a non-`undefined` value.
